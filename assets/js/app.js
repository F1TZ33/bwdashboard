...
(function(){
  // Repo-root safe base for GitHub Pages (handles /bwdashboard/ and nested /products/ pages)
function __bwRepoBase(){
  var parts = (location.pathname || "/").split("/").filter(Boolean);
  if(parts.length && parts[parts.length-1].includes(".")) parts.pop(); // drop filename
  // assume first segment is repo name for GitHub Pages: /<repo>/...
  return "/" + (parts[0] || "") + "/";
}
const SUPPLIERS_URL = new URL(__bwRepoBase() + "docs/suppliers.json", location.origin).href;
